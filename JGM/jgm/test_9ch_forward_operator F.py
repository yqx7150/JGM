...
__all__ = ['Test_9ch_forward_operator F']

def write_Data(result_all_13,i,z):
    with open(os.path.join('./oldpic/output_result/model1/1',"psnr_output_{}".format(z)+".txt"),"w+") as f:
...
def write_Data_finally(result_all,i):
    with open(os.path.join('./laotupian/output/',"psnr_output_finally"+".txt"),"w+") as f:
        for i in range(len(result_all)):
            f.writelines('current image {} PSNR : '.format(i) + str(result_all[i][0]) + '    SSIM : ' + str(result_all[i][1]))
            f.write('\n')

class Test_9ch_forward_operator():
    ...
    def test(self):
        ...
        states = torch.load(os.path.join(self.args.log, 'checkpoint_440000.pth'), map_location=self.config.device)   #coco checkpoint_140000
        ...
        batch_size = 1 
        samples = 1
        files_list = glob.glob('./oldpic/8.png')
        ...
        for z,file_path in enumerate(files_list):
            for c in range(12):
                ...
                #gray = ((0.3*img[:,0,...] +  0.59*img[:,1,...] + 0.11*img[:,2,...])).cuda()
                gray_mixed = torch.stack([gray,gray,gray],dim=1)

                u0_h = np.zeros([1,size,size,3],dtype=np.float32)
                u0_v = np.zeros([1,size,size,3],dtype=np.float32)
                ...
                step_lr=0.0000599 * 0.26   #coco 0.0000599 * 0.26
                ...
                for idx, sigma in enumerate(sigmas):
                    ...
                    for step in range(n_steps_each):
                        ...
                        '''x0_mix = (0.3*x01[:,0,...] + 0.59*x01[:,1,...] + 0.11*x01[:,2,...])
                        x0_mix_grad_h = (0.3*x01[:,3,...] + 0.59*x01[:,4,...] + 0.11*x01[:,5,...])
                        x0_mix_grad_v = (0.3*x01[:,6,...] + 0.59*x01[:,7,...] + 0.11*x01[:,8,...])'''
                        ...
                        x0_U = np.zeros([1,size,size,3],dtype=np.float32)
            ...
            files_list_12 = glob.glob('./laotupian/img_{}_Rec_9ch_x_end_rgb_{}.png'.format(z,np.argmax(result_all_12[:,0],0)))
            shutil.copy(files_list_12[0],'./laotupian/output/')
